...
volatile uint8_t disp_ready = 1; // 最初は転送可能状態
static uint8_t current_page = 0;           // 現在送信中のページ
...
#error "Only 32, 64, or 128 lines of height are supported!"
    ...
    printf("truly first updatescren\n");
    ssd1306_UpdateScreen();
    printf("truly first updatescren finished\n");
...
#else
void ssd1306_UpdateScreen(void) {
    //if(!disp_ready) return; // DMA送信中は待つ
    while(!disp_ready);

    disp_ready = 0;
    current_page = 0;
    ssd1306_SendPage_DMA(current_page);
}

void ssd1306_SendPage_DMA(uint8_t page) {
    static uint8_t data[129];
    data[0] = 0x40; // D/C# = データモード
    memcpy(&data[1], &SSD1306_Buffer[page*128], 128);

    // ページアドレスを先に送信する（ブロック送信でもOK）
    ssd1306_WriteCommand(0xB0 + current_page); // Set the current RAM page address.
    ssd1306_WriteCommand(0x00 + SSD1306_X_OFFSET_LOWER);
    ssd1306_WriteCommand(0x10 + SSD1306_X_OFFSET_UPPER);

    //printf("pagedata Transmitted\n");
    // データ部分をDMAで送信
    while(HAL_I2C_GetState(&hi2c2) != HAL_I2C_STATE_READY);
    HAL_I2C_Mem_Write_DMA(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, 0x40, 1, &SSD1306_Buffer[SSD1306_WIDTH*current_page], SSD1306_WIDTH);
}
...
void HAL_I2C_MemTxCpltCallback(I2C_HandleTypeDef *hi2c)
{
    if (hi2c->Instance == I2C2) {
        ...
        if(current_page < SSD1306_HEIGHT/8) {
            ssd1306_SendPage_DMA(current_page); // 次ページを送信
        } else {
            disp_ready = 1; // 全ページ送信完了
        }
    }
}
...
char ssd1306_WriteChar(char ch, SSD1306_Font_t Font, SSD1306_COLOR color) {
    ...
    if(Font.width==32){
        for(i = 0; i < Font.height; i++) {
            b = Font.data[(ch - 48) * Font.height*2 + 2*i]<<16 | Font.data[(ch - 48) * Font.height*2 + 2*i+1];
            //printf("%lx\r\n", b);
            for(j = 0; j < 32; j++) {
                if((b << j) & 0x80000000)  {
...
char ssd1306_WriteString(char* str, SSD1306_Font_t Font, SSD1306_COLOR color) {
    while (*str) {
        //printf("writestring font.width : %d\r\n", Font.width);
